...
void lfb_init()
{
    ...
    if (mbox_call(MBOX_CH_PROP) && mbox[20] == 32 && mbox[28] != 0)
    {
        ...
        phy_width = mbox[5];    // get actual physical width
        phy_height = mbox[6];   // get actual physical height
        pitch = mbox[33];       // get number of bytes per line
        isrgb = mbox[24];       // get the actual channel order
        lfb = (void *)P2V((unsigned long)mbox[28]);
    }
    else
    {
...
void lfb_showpicture()
{
    ...
    ptr += (phy_height - height) / 2 * pitch + (phy_width - width) * 2;
    for (y = 0; y < height; y++)
    {
        for (x = 0; x < width; x++)
        {
...
void lfb_showpicture_gato()
{
    int x, y;
    unsigned char *ptr = lfb;
    char *data = gato_data, pixel[4];

    ptr += (phy_height - gato_height) / 2 * pitch + (phy_width - gato_width) * 2;
    for (y = 0; y < gato_height; y++)
    {
        for (x = 0; x < width; x++)
        {
            HEADER_PIXEL(data, pixel);
