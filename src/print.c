...
static int decimal_to_string(char *buffer, int position, uint64_t digits)
{
    ...
    return size + udecimal_to_string(buffer, position, (uint64_t)digits);
}
...
int printk(const char *format, ...)
{
    ...
    for (int i = 0; format[i] != '\0'; i++)
    {
        ...
        else
        {
            ...
            case 'd':
                integer = va_arg(args, int64_t);
                buffer_size += decimal_to_string(buffer, buffer_size, (uint64_t)integer);
                break;
                break;
            case 's':
